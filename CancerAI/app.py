...
model = keras.models.load_model(f'{modelPath}\\HistoCNN.model')
typ = 'h'
...
def submit():
	if request.method == 'POST':
		...
		if request.form.get('Blood Cell'):
			model = keras.models.load_model(f'{modelPath}\\BloodNN.model')
			typ = 'b'
		if request.form.get('Leukemia'):
			model = keras.models.load_model(f'{modelPath}\\LeukNN.model')
			typ = 'l'
	return ''
